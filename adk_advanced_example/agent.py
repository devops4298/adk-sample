...
async def after_agent_callback_with_persistence(
    ...
    if session_id:
        state_service.update_step(
            ...
            status="completed",
            data={"message_count": callback_context.state.get("message_count", 0)}
        )
...
def create_advanced_agent() -> Agent:
    ...
    # Collect tools
    # Note: google_search cannot be mixed with FunctionTools in Vertex AI
    # Use either search tools OR function tools, not both
    tools = [
